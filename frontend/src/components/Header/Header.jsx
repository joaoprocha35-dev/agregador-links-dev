...
// Mapeio os ícones de redes sociais vetorizados
const iconesMap = {
  Github: () => (
...
export const Header = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Passo 1: Apresento a imagem partindo de opacidade 0 e escala reduzida
      tl.from(`.${styles.avatarContainer}`, {
        scale: 0.5,
        opacity: 0,
        duration: 0.5,
        ease: 'back.out(1.7)',
      })
      // Passo 2: Elevo o título e a etiqueta funcional partindo de opacidade 0
      .from([`.${styles.nome}`, `.${styles.cargoWrapper}`], {
        y: 16,
        opacity: 0,
        duration: 0.45,
        stagger: 0.12,
      ...
      // Passo 3: Distribuo os atalhos sociais partindo de opacidade e escala 0
      .from(`.${styles.socialLink}`, {
        y: 18,
        opacity: 0,
        scale: 0,
        duration: 0.35,
  ...
  return (
    <header ref={headerRef} className={styles.header}>
      ...
      <nav className={styles.socialNav}>
        {usuarioInfo.redesSociais.map((rede) => {
